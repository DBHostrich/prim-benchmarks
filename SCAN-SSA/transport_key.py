#!/usr/bin/env python3
"""Canonical v9 keys for SCAN collection transfers."""
...
def logical_distribution_class(op: str, subop: str) -> str:
    ...
    if subop in {"input_arguments_add", "input_data"}:
        return "PARTITIONED_SCATTER"
    if subop in {"partial_results", "output_data"}:
...
def same_source_across_group(op: str, subop: str) -> str:
    if op not in TRANSFER_OPS:
        return ""
    return "1" if subop == "input_arguments_scan" else "0"
...
def previous_sdk_op_class(row: Mapping[str, str]) -> str:
    ...
    if operation == "dpu_push_xfer":
        ...
        if direction == "FROM_DPU" and target_space == "WRAM":
