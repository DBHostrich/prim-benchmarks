...
RESULT_ROOT="${RESULT_ROOT:-/tmp/bdang/scan_ssa_v9_numa0_$(date +%Y%m%d_%H%M%S)}"
LATEST_RESULT_POINTER="${LATEST_RESULT_POINTER:-/tmp/bdang/latest_scan_ssa_v9_numa0_path.txt}"
NUMA_NODE="${NUMA_NODE:-0}"
EXCLUDED_SYSFS_RANKS="${EXCLUDED_SYSFS_RANKS:-4,5}"
EXCLUDED_SYSFS_RANKS_REASON="${EXCLUDED_SYSFS_RANKS_REASON:-rank4_scan_output_mismatch_rank5_mram_mux_timeout}"
DPUS_LIST="${DPUS_LIST:-64 128 256 512 1024 1152}"
TASKLETS_LIST="${TASKLETS_LIST:-16}"
INPUT_SIZE="${INPUT_SIZE:-251658240}"
SCALING_MODE="${SCALING_MODE:-1}"
BLOCK_LOG2="${BLOCK_LOG2:-10}"
...
if [[ "$INPUT_SIZE" != "251658240" || "$SCALING_MODE" != "1" ]]; then
    echo "ERROR: the validator defines SCAN-SSA strong scaling with INPUT_SIZE=251658240" >&2
    exit 1
fi
if [[ "$TASKLETS_LIST" != "16" || "$BLOCK_LOG2" != "10" ]]; then
    echo "ERROR: the validator defines NR_TASKLETS=16 and BL=10" >&2
    exit 1
fi
...
export SCAN_TRACE_DPU_RANK_TOPOLOGY_TSV="$RESULT_ROOT/dpu_rank_topology.tsv"
...
for nr_dpus in $DPUS_LIST; do
    ...
    rank_paths="$(python3 "$SCRIPT_DIR/select_rank_paths.py" \
        "$SCAN_TRACE_DPU_RANK_TOPOLOGY_TSV" \
        --numa-node "$NUMA_NODE" \
...
config_name() {
    ...
    remainder="${spec#*:}"
    tasklets="${remainder%%:*}"
    printf 'SCAN_%sdpu_%stl' "$nr_dpus" "$tasklets"
}
...
run_scan() {
    numactl --cpunodebind="$NUMA_NODE" --membind="$NUMA_NODE" \
        ./bin/host_code -i "$INPUT_SIZE" -x "$SCALING_MODE" \
...
require_correct_result() {
    ...
    if ! grep -q "Outputs are equal" "$log_path"; then
        echo "ERROR: SCAN verification marker is absent in $log_path" >&2
        exit 1
    fi
    if grep -q "Outputs differ" "$log_path"; then
        echo "ERROR: SCAN verification mismatch in $log_path" >&2
        exit 1
    fi
}

echo "==> Building ${#configs[@]} SCAN configurations before collection"
for spec in "${configs[@]}"; do
    ...
    make clean
    make NR_DPUS="$nr_dpus" NR_TASKLETS="$tasklets" BL="$BLOCK_LOG2" \
        TYPE=INT64 all \
    ...
    printf 'NR_DPUS=%s\nNR_TASKLETS=%s\nINPUT_SIZE=%s\nSCALING_MODE=STRONG\nBLOCK_LOG2=%s\nTYPE=INT64\nNUMA_NODE=%s\nEXCLUDED_SYSFS_RANKS=%s\nEXCLUDED_SYSFS_RANKS_REASON=%s\nSCAN_DPU_RANK_PATHS=%s\nEXPECTED_SYSFS_RANKS=%s\nEXPECTED_CHANNELS=%s\nPROCESS_WARMUP_RUNS=%s\nTRACE_RUNS=%s\nIN_PROCESS_WARMUP=%s\nIN_PROCESS_REPS=%s\nTRANSPORT_KEY_VERSION=%s\nTIMING_SCOPE=PUSH_ONLY\nCOLLECTION_ORDER=LATIN_ROTATION\n' \
        "$nr_dpus" "$tasklets" "$INPUT_SIZE" "$BLOCK_LOG2" "$NUMA_NODE" \
        "$EXCLUDED_SYSFS_RANKS" "$EXCLUDED_SYSFS_RANKS_REASON" \
        "$rank_paths" "$sysfs_ranks" "$channels" \
...
run_rounds() {
    ...
    for rep in $(seq 1 "$rounds"); do
        ...
        for slot in $(seq 0 $((config_count - 1))); do
            ...
            export SCAN_DPU_RANK_PATHS="$rank_paths"

            unset SCAN_TRACE_CSV SCAN_TRACE_DPUS_CSV SCAN_TRACE_RUN_ID \
                SCAN_TRACE_REPEAT_ID SCAN_TRACE_HOST_NUMA_NODE \
                SCAN_TRACE_PROCESS_STATE SCAN_TRACE_PREWARM_RUNS \
                SCAN_TRACE_HOST_BINDING_MODE SCAN_TRACE_HOST_CPU_LIST || true
            if [[ "$phase" == "trace" ]]; then
                export SCAN_TRACE_CSV="$result_dir/trace_${rep_id}.csv"
                export SCAN_TRACE_DPUS_CSV="$result_dir/trace_${rep_id}_dpus.csv"
                export SCAN_TRACE_RUN_ID="$name"
                export SCAN_TRACE_REPEAT_ID="$rep"
                export SCAN_TRACE_HOST_NUMA_NODE="$NUMA_NODE"
                export SCAN_TRACE_PROCESS_STATE="interleaved_fresh_process"
                export SCAN_TRACE_HOST_BINDING_MODE="NODE_ONLY"
                export SCAN_TRACE_HOST_CPU_LIST="numa_node_${NUMA_NODE}"
                export SCAN_TRACE_PREWARM_RUNS="$PROCESS_WARMUP_RUNS"
    ...
    unset SCAN_DPU_RANK_PATHS SCAN_TRACE_CSV SCAN_TRACE_DPUS_CSV \
        SCAN_TRACE_RUN_ID SCAN_TRACE_REPEAT_ID SCAN_TRACE_HOST_NUMA_NODE \
        SCAN_TRACE_PROCESS_STATE SCAN_TRACE_PREWARM_RUNS \
        SCAN_TRACE_HOST_BINDING_MODE SCAN_TRACE_HOST_CPU_LIST || true
}
