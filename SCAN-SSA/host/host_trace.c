...
struct ScanPhysicalRankTopology {
...
static bool lookup_physical_rank_topology(
	const char *path,
	uint32_t sdk_rank_id,
	struct ScanPhysicalRankTopology *topology
) {
...
static const char *sdk_api_kind(const struct ScanHostTraceEvent *event) {
	return event->has_transfer ? "PUSH_XFER" : "";
}

static const char *logical_distribution_class(
	const struct ScanHostTraceEvent *event
) {
	if (!event->has_transfer)
		return "";
	if (strcmp(event->subop, "input_arguments_scan") == 0)
		return "SHARED_REPLICATION";
	if (strcmp(event->subop, "input_arguments_add") == 0
		|| strcmp(event->subop, "input_data") == 0)
		return "PARTITIONED_SCATTER";
	if (strcmp(event->subop, "partial_results") == 0
		|| strcmp(event->subop, "output_data") == 0)
...
static const char *same_source_across_group(
	...
	if (!event->has_transfer)
		return "";
	return strcmp(event->subop, "input_arguments_scan") == 0 ? "1" : "0";
}

static const char *phase_class(const struct ScanHostTraceEvent *event) {
	if (!event->has_transfer)
		return "";
...
static const char *previous_sdk_op_class(
	const struct ScanHostTraceEvent *previous
) {
	if (previous == NULL)
		return "NONE";
	...
	if (strcmp(previous->op, "dpu_push_xfer") == 0) {
		...
		if (strcmp(previous->direction, "FROM_DPU") == 0
			&& strcmp(previous->target_space, "WRAM") == 0)
			return "PUSH_XFER_FROM_DPU_WRAM";
		if (strcmp(previous->direction, "FROM_DPU") == 0
			&& strcmp(previous->target_space, "MRAM") == 0)
...
static void grow_events(struct ScanHostTrace *trace) {
	...
	if (new_capacity < trace->event_capacity
		...
		fprintf(stderr, "SCAN host event trace capacity overflow\n");
		exit(EXIT_FAILURE);
	}
	events = realloc(trace->events, new_capacity * sizeof(*trace->events));
	if (events == NULL) {
		fprintf(stderr, "Could not grow the SCAN event trace\n");
		exit(EXIT_FAILURE);
	}
...
bool scan_host_trace_requested(void) {
	const char *path = getenv("SCAN_TRACE_CSV");
...
static void read_runtime_counters(
	...
	if (usage.ru_nvcsw < 0 || usage.ru_nivcsw < 0
		...
		fprintf(stderr, "Negative SCAN runtime diagnostic counter\n");
		exit(EXIT_FAILURE);
	}
	*cpu_id = cpu;
...
void scan_host_trace_measurement_begin(
	struct ScanHostTraceMeasurement *measurement
) {
	...
	measurement->wall_start_ns = scan_host_trace_now_ns();
	measurement->thread_cpu_start_ns = thread_cpu_now_ns();
}

void scan_host_trace_measurement_end(
	struct ScanHostTraceMeasurement *measurement
) {
	measurement->thread_cpu_end_ns = thread_cpu_now_ns();
	measurement->wall_end_ns = scan_host_trace_now_ns();
	read_runtime_counters(
...
static void set_runtime_diagnostics(
	struct ScanHostTraceEvent *event,
	...
	if (measurement == NULL
		...
		fprintf(stderr, "Invalid SCAN runtime diagnostic interval\n");
		exit(EXIT_FAILURE);
	}
...
bool scan_host_trace_init(
	...
	const char *events_path = getenv("SCAN_TRACE_CSV");
	const char *dpus_path = getenv("SCAN_TRACE_DPUS_CSV");
	const char *run_id = getenv("SCAN_TRACE_RUN_ID");
	const char *repeat_id = getenv("SCAN_TRACE_REPEAT_ID");
	const char *host_numa_node = getenv("SCAN_TRACE_HOST_NUMA_NODE");
	const char *process_state = getenv("SCAN_TRACE_PROCESS_STATE");
	const char *host_binding_mode = getenv("SCAN_TRACE_HOST_BINDING_MODE");
	const char *host_cpu_list = getenv("SCAN_TRACE_HOST_CPU_LIST");
	const char *pretrace_warmup_runs = getenv("SCAN_TRACE_PREWARM_RUNS");
	const char *topology_path = getenv("SCAN_TRACE_DPU_RANK_TOPOLOGY_TSV");
	struct dpu_set_t rank = {0};
	struct dpu_set_t dpu = {0};
	struct dpu_rank_t *sdk_rank;
	struct ScanPhysicalRankTopology physical = {0};
	...
	if (!scan_host_trace_requested())
		return true;
	if (dpus_path == NULL || dpus_path[0] == '\0') {
		fprintf(stderr, "SCAN_TRACE_DPUS_CSV is required with SCAN_TRACE_CSV\n");
		return false;
	}
	if (topology_path == NULL || topology_path[0] == '\0') {
		fprintf(stderr,
			"SCAN_TRACE_DPU_RANK_TOPOLOGY_TSV is required for v9 labels\n");
		return false;
	}

	trace->events_output_path = events_path;
	trace->dpus_output_path = dpus_path;
	trace->run_id = run_id == NULL || run_id[0] == '\0' ? "scan" : run_id;
	trace->configured_dpus = configured_dpus;
	trace->num_tasklets = num_tasklets;
	trace->input_size = input_size;
	trace->input_size_per_dpu = input_size_per_dpu;
	trace->input_size_per_dpu_round = input_size_per_dpu_round;
	trace->scaling_mode = scaling_mode;
	...
	if (!parse_unsigned_env("SCAN_TRACE_REPEAT_ID", repeat_id, &trace->repeat_id)
		|| !parse_unsigned_env("SCAN_TRACE_PREWARM_RUNS", pretrace_warmup_runs,
	...
	if (!is_label_atom(trace->host_numa_node)
		...
		|| !is_label_atom(trace->scaling_mode)) {
		fprintf(stderr, "SCAN trace NUMA/process label contains unsupported characters\n");
		return false;
	}
	if (dpu_get_nr_ranks(dpu_set, &trace->actual_ranks) != DPU_OK
		|| trace->actual_ranks == 0 || configured_dpus == 0) {
		fprintf(stderr, "Could not query a valid SCAN allocation shape\n");
		return false;
	}
	...
	if (trace->rank_ordinals == NULL || trace->dpu_ids_in_rank == NULL
		...
		|| rank_counts == NULL) {
		fprintf(stderr, "Could not allocate SCAN trace buffers\n");
		free(rank_counts);
		scan_host_trace_destroy(trace);
		return false;
	}
	...
	DPU_RANK_FOREACH(dpu_set, rank, rank_ordinal) {
		...
		if (rank_ordinal >= trace->actual_ranks) {
			fprintf(stderr, "Allocated SCAN rank ordinal exceeds rank count\n");
			free(rank_counts);
			scan_host_trace_destroy(trace);
			return false;
		}
		...
		if (!append_text(trace->dpu_rank_numa_nodes_text, rank_text_capacity,
			...
			fprintf(stderr, "Could not format SCAN topology signature\n");
			free(rank_counts);
			scan_host_trace_destroy(trace);
			return false;
		}
		...
		DPU_FOREACH(rank, dpu, dpu_id_in_rank) {
			if (global_dpu_id >= configured_dpus) {
				fprintf(stderr, "Allocated SCAN topology exceeds configured DPUs\n");
				free(rank_counts);
				scan_host_trace_destroy(trace);
				return false;
			}
	...
	if (global_dpu_id != configured_dpus) {
		fprintf(stderr, "Allocated SCAN topology has %u DPUs, expected %u\n",
	...
	for (rank_ordinal = 0; rank_ordinal < trace->actual_ranks; ++rank_ordinal) {
		if (!append_text(trace->active_dpus_per_rank, rank_text_capacity,
				...
				rank_counts[rank_ordinal])) {
			fprintf(stderr, "Could not format SCAN rank shape\n");
			free(rank_counts);
			scan_host_trace_destroy(trace);
			return false;
		}
...
void scan_host_trace_record_event(
	...
	struct ScanHostTraceEvent *event;
	if (!scan_host_trace_enabled(trace))
		return;
...
void scan_host_trace_record_transfer(
	...
	if (size_per_dpu_bytes > UINT64_MAX / trace->configured_dpus) {
		fprintf(stderr, "Invalid SCAN transfer trace interval or byte count\n");
		exit(EXIT_FAILURE);
	}
	for (dpu_id = 0; dpu_id < trace->configured_dpus; ++dpu_id) {
		...
		if (logical_bytes > size_per_dpu_bytes
			...
			fprintf(stderr, "Invalid SCAN logical bytes for DPU %u\n", dpu_id);
			exit(EXIT_FAILURE);
		}
...
static bool format_transport_key(
	const struct ScanHostTrace *trace,
	const struct ScanHostTraceEvent *event,
	char *output,
	size_t output_size
) {
...
static bool write_events(const struct ScanHostTrace *trace) {
	FILE *stream = fopen(trace->events_output_path, "w");
	size_t index;
	if (stream == NULL) {
		fprintf(stderr, "Could not open SCAN event trace %s: %s\n",
			trace->events_output_path, strerror(errno));
		return false;
	}
	fputs(
		"run_id,repeat_id,event_id,configured_dpus,actual_ranks,num_tasklets,"
		"input_size,input_size_per_dpu,input_size_per_dpu_round,scaling_mode,"
		"op,direction,sdk_api_kind,"
		...
		"host_cpu_list,algorithm_variant,scaling_mode_label,"
	...
	for (index = 0; index < trace->num_events; ++index) {
		const struct ScanHostTraceEvent *event = &trace->events[index];
		const struct ScanHostTraceEvent *previous = index == 0
			? NULL : &trace->events[index - 1];
		char transport_key[2048];

		if (!format_transport_key(trace, event, transport_key,
				sizeof(transport_key))) {
			fprintf(stderr, "SCAN v9 transport key is too long\n");
			fclose(stream);
			return false;
		}
		write_csv_string(stream, trace->run_id);
		fprintf(stream, ",%" PRIu64 ",%" PRIu64 ",%u,%u,%u,%" PRIu64
			",%u,%u,",
			trace->repeat_id, event->event_id, trace->configured_dpus,
			trace->actual_ranks, trace->num_tasklets, trace->input_size,
			trace->input_size_per_dpu, trace->input_size_per_dpu_round);
		write_csv_string(stream, trace->scaling_mode);
		fputc(',', stream);
		...
		fputc(',', stream);
		write_csv_string(stream, "SCAN_SSA");
		fputc(',', stream);
		write_csv_string(stream, trace->scaling_mode);
		fputc(',', stream);
		if (event->has_transfer) {
	...
	if (fclose(stream) != 0) {
		fprintf(stderr, "Could not close SCAN event trace %s: %s\n",
			trace->events_output_path, strerror(errno));
		return false;
	}
	return true;
...
static bool write_dpu_rows(const struct ScanHostTrace *trace) {
	FILE *stream = fopen(trace->dpus_output_path, "w");
	size_t index;
	if (stream == NULL) {
		fprintf(stderr, "Could not open SCAN DPU trace %s: %s\n",
	...
	for (index = 0; index < trace->num_dpu_rows; ++index) {
		const struct ScanHostTraceDpu *row = &trace->dpu_rows[index];
	...
	if (fclose(stream) != 0) {
		fprintf(stderr, "Could not close SCAN DPU trace %s: %s\n",
			trace->dpus_output_path, strerror(errno));
		return false;
	}
	return true;
