...
RESULT_ROOT="${RESULT_ROOT:-/dev/shm/bfs_context_probe_$(date +%Y%m%d_%H%M%S)}"
LATEST_RESULT_POINTER="${LATEST_RESULT_POINTER:-/dev/shm/latest_bfs_context_probe_path.txt}"
NUMA_NODE="${NUMA_NODE:-0}"
NR_DPUS="${NR_DPUS:-256}"
NR_TASKLETS="${NR_TASKLETS:-1}"
TARGET_DPU_LIST="${TARGET_DPU_LIST:-0}"
N_PROCESS_REPS="${N_PROCESS_REPS:-10}"
SAMPLES_PER_PROCESS="${SAMPLES_PER_PROCESS:-60}"
WARMUPS_PER_PROCESS="${WARMUPS_PER_PROCESS:-5}"
...
printf '%s\n' \
    ...
    "NUMA_NODE=$NUMA_NODE" \
    "TARGET_DPU_LIST=$TARGET_DPU_LIST" \
...
echo "==> Building BFS context probe: ${NR_DPUS} DPUs, ${NR_TASKLETS} tasklet(s)"
make clean
...
for target_dpu in $TARGET_DPU_LIST; do
    target_dir="$RESULT_ROOT/dpu_${target_dpu}"
    mkdir -p "$target_dir"
    for process_rep in $(seq 1 "$N_PROCESS_REPS"); do
        rep_id="$(printf '%02d' "$process_rep")"
        export BFS_CONTEXT_PROBE_CSV="$target_dir/probe_${rep_id}.csv"
        export BFS_CONTEXT_PROBE_RUN_ID="BFS_${NR_DPUS}dpu_${NR_TASKLETS}tl_dpu${target_dpu}"
        export BFS_CONTEXT_PROBE_PROCESS_REPEAT="$process_rep"
        export BFS_CONTEXT_PROBE_SAMPLES="$SAMPLES_PER_PROCESS"
        export BFS_CONTEXT_PROBE_WARMUPS="$WARMUPS_PER_PROCESS"
        export BFS_CONTEXT_PROBE_DPU="$target_dpu"
        run_log="$target_dir/run_${rep_id}.log"
        echo "==> target DPU $target_dpu, fresh process $process_rep/$N_PROCESS_REPS"
        run_probe > "$run_log" 2>&1
        grep -q "Context probe wrote $SAMPLES_PER_PROCESS samples per condition" \
            "$run_log"
    done
done

unset BFS_CONTEXT_PROBE_CSV BFS_CONTEXT_PROBE_RUN_ID \
    BFS_CONTEXT_PROBE_PROCESS_REPEAT BFS_CONTEXT_PROBE_SAMPLES \
    BFS_CONTEXT_PROBE_WARMUPS BFS_CONTEXT_PROBE_DPU || true

mapfile -t traces < <(
    find "$RESULT_ROOT" -mindepth 2 -maxdepth 2 -type f -name 'probe_*.csv' | sort
)
if [[ "${#traces[@]}" -eq 0 ]]; then
    echo "ERROR: context probe produced zero CSV traces" >&2
    exit 1
fi

python3 "$SCRIPT_DIR/analyze_context_probe.py" \
    ...
    --summary-output "$RESULT_ROOT/condition_summary.csv" \
    --paired-output "$RESULT_ROOT/paired_effects.csv" \
    "${traces[@]}" \
    | tee "$RESULT_ROOT/analysis.log"

printf '%s\n' "$RESULT_ROOT" | tee "$LATEST_RESULT_POINTER"
echo "Context probe results: $RESULT_ROOT"
