...
RESULT_ROOT="${RESULT_ROOT:-/dev/shm/bfs_group_context_probe_$(date +%Y%m%d_%H%M%S)}"
LATEST_RESULT_POINTER="${LATEST_RESULT_POINTER:-/dev/shm/latest_bfs_group_context_probe_path.txt}"
...
echo "==> Building BFS group context probe: ${NR_DPUS} DPUs, ${NR_TASKLETS} tasklet(s)"
make clean
...
unset BFS_TRACE_CSV BFS_TRACE_RUN_ID BFS_TRACE_REPEAT_ID \
    ...
    BFS_CONTEXT_PROBE_DPU || true

for process_rep in $(seq 1 "$N_PROCESS_REPS"); do
    rep_id="$(printf '%02d' "$process_rep")"
    export BFS_GROUP_CONTEXT_PROBE_CSV="$RESULT_ROOT/probe_${rep_id}.csv"
    export BFS_GROUP_CONTEXT_PROBE_RUN_ID="BFS_${NR_DPUS}dpu_${NR_TASKLETS}tl_group"
    export BFS_GROUP_CONTEXT_PROBE_PROCESS_REPEAT="$process_rep"
    export BFS_GROUP_CONTEXT_PROBE_SAMPLES="$SAMPLES_PER_PROCESS"
    export BFS_GROUP_CONTEXT_PROBE_WARMUPS="$WARMUPS_PER_PROCESS"
    run_log="$RESULT_ROOT/run_${rep_id}.log"
    echo "==> fresh process $process_rep/$N_PROCESS_REPS"
    run_probe > "$run_log" 2>&1
    grep -q \
        "Group context probe wrote $SAMPLES_PER_PROCESS samples per condition" \
        "$run_log"
done

unset BFS_GROUP_CONTEXT_PROBE_CSV BFS_GROUP_CONTEXT_PROBE_RUN_ID \
    BFS_GROUP_CONTEXT_PROBE_PROCESS_REPEAT BFS_GROUP_CONTEXT_PROBE_SAMPLES \
    BFS_GROUP_CONTEXT_PROBE_WARMUPS || true

mapfile -t traces < <(
    find "$RESULT_ROOT" -maxdepth 1 -type f -name 'probe_*.csv' | sort
)
if [[ "${#traces[@]}" -eq 0 ]]; then
    echo "ERROR: group context probe produced zero CSV traces" >&2
    exit 1
fi

python3 "$SCRIPT_DIR/analyze_group_context_probe.py" \
...
echo "Group context probe results: $RESULT_ROOT"
