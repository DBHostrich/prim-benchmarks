...
RESULT_ROOT="${RESULT_ROOT:-/dev/shm/bfs_predecessor_probe_$(date +%Y%m%d_%H%M%S)}"
LATEST_RESULT_POINTER="${LATEST_RESULT_POINTER:-/dev/shm/latest_bfs_predecessor_probe_path.txt}"
...
echo "==> Building BFS predecessor factorial probe: ${NR_DPUS} DPUs, ${NR_TASKLETS} tasklet(s)"
make clean
...
unset BFS_TRACE_CSV BFS_TRACE_RUN_ID BFS_TRACE_REPEAT_ID \
    ...
    BFS_GROUP_CONTEXT_PROBE_SAMPLES BFS_GROUP_CONTEXT_PROBE_WARMUPS \
    BFS_API_ORDER_PROBE_CSV BFS_API_ORDER_PROBE_RUN_ID \
...
for process_rep in $(seq 1 "$N_PROCESS_REPS"); do
    ...
    export BFS_PREDECESSOR_PROBE_CSV="$RESULT_ROOT/probe_${rep_id}.csv"
    export BFS_PREDECESSOR_PROBE_RUN_ID="BFS_${NR_DPUS}dpu_${NR_TASKLETS}tl_predecessor_factorial"
    export BFS_PREDECESSOR_PROBE_PROCESS_REPEAT="$process_rep"
    export BFS_PREDECESSOR_PROBE_SAMPLES="$SAMPLES_PER_PROCESS"
    export BFS_PREDECESSOR_PROBE_WARMUPS="$WARMUPS_PER_PROCESS"
    run_log="$RESULT_ROOT/run_${rep_id}.log"
    echo "==> fresh process $process_rep/$N_PROCESS_REPS"
    run_probe > "$run_log" 2>&1
    grep -q \
        "Predecessor probe wrote $SAMPLES_PER_PROCESS samples per condition" \
        "$run_log"
done

unset BFS_PREDECESSOR_PROBE_CSV BFS_PREDECESSOR_PROBE_RUN_ID \
    BFS_PREDECESSOR_PROBE_PROCESS_REPEAT BFS_PREDECESSOR_PROBE_SAMPLES \
    BFS_PREDECESSOR_PROBE_WARMUPS || true

mapfile -t traces < <(
    find "$RESULT_ROOT" -maxdepth 1 -type f -name 'probe_*.csv' | sort
)
if [[ "${#traces[@]}" -eq 0 ]]; then
    echo "ERROR: predecessor probe produced zero CSV traces" >&2
    exit 1
fi

python3 "$SCRIPT_DIR/analyze_predecessor_probe.py" \
    ...
    --condition-output "$RESULT_ROOT/condition_summary.csv" \
    --per-dpu-condition-output "$RESULT_ROOT/per_dpu_condition_summary.csv" \
    --overall-effects-output "$RESULT_ROOT/overall_factorial_effects.csv" \
    --parity-effects-output "$RESULT_ROOT/parity_factorial_effects.csv" \
    --position-effects-output "$RESULT_ROOT/position_factorial_effects.csv" \
    --per-dpu-effects-output "$RESULT_ROOT/per_dpu_factorial_effects.csv" \
    --topology-output "$RESULT_ROOT/topology_map.csv" \
    "${traces[@]}" \
    | tee "$RESULT_ROOT/analysis.log"

printf '%s\n' "$RESULT_ROOT" | tee "$LATEST_RESULT_POINTER"
echo "Predecessor factorial probe results: $RESULT_ROOT"
